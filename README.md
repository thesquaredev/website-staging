...
## Migrate from staging to live environment and vice versa
...
1.  **Merge from staging repo to live repo**
    ...
    # Merge remote master branch (or the branch you want to merge) to local master branch
    git merge website-staging/master
    ```

2.  **Merge from live repo to stage repo**

    ```
    # Add live remote to staging repo (only needed once)
    git remote add website https://github.com/thesquaredev/website.git
    git remote update

    # Merge remote master branch (or the branch you want to merge) to local master branch
    git merge website/master
    ```

3.  **Change custom domain for GitHub Pages**
